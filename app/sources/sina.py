...
def kline(symbol: str, period: str = "day", limit: int = 320,
    ...
    **重要：这里返回的是不复权（原始）价格，新浪这个接口没有前复权。**
    实测与腾讯前复权的差异不小 —— 600519 近 300 天中位差 2.1%、
    000001.SZ 中位差 3.3%（最大 5.6%），而箱体高度往往只有 10% 出头，
    这种系统性偏移足以改变箱体边界和均线位置。
    所以请求复权数据的调用方**不应该用这个函数兜底** ——
    给一份价格口径不对的数据，比少给一份数据更糟。
    """
