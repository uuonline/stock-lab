...
- **箱体分析**：识别股价震荡区间，在 K线图上画出箱顶（压力）/箱底（支撑）
  ...
  非震荡形态会明确标注并降低置信度
- **按个股节奏自适应定窗口**：见下节。固定窗口用同一把尺子量所有股票，
  而每只股票的运作节奏差很远，窗口选错会得出完全相反的结论
...
把「让 AI 做信息整理和逻辑推演、判断留给人」这套流程做成栏目。核心不是提示词，
而是**原料必须是真实数据** —— AI 在「竞争对手是谁」这类通用知识上可靠，
在「单一客户占比多少」这类具体数字上会编。所以：

| 档位 | 数量 | 含义 |
|---|---|---|
| 有数据 | 7 | 结论全部由系统真实数据算出，并标注算法与来源 |
| 部分 | 2 | 一部分能答，缺口明确标出 |
| 无数据 | 4 | 本系统确实没有这类数据，**不生成任何结论**，只给出查询途径 |

**后来发现「无数据」判早了。** 用户追问「为什么没有数据」之后重新去找，发现东财的
另一个域名族（`emweb.securities.eastmoney.com` / `datacenter-web.eastmoney.com`）
有这些数据的结构化接口 —— 它和行情用的 `push2*` 是两回事，`push2` 整族被限流时
这几个接口仍然正常。补齐后 A股从「有数据 7」变成「**有数据 10 / 部分 2 / 无数据 1**」：

| 提示词 | 新增数据源 | 内容 |
|---|---|---|
| #1 商业模式与收入来源 | F10 经营分析 | 主营构成（按产品/行业/地区，带收入、占比、毛利率）+ 公司自述经营评述原文 |
| #2 行业竞争对手 | F10 行业分析 | **三份不同的同行榜单** + 行业市值/营收/净利排名 |
| #5 市净率与 ROE 对比 | F10 行业分析 | 同行估值榜单 + 行业平均 PE/PB + 行业中值 ROE/净利率 |
| #9 股东高管增减持 | 东财数据中心 | 逐笔的高管持股变动 + 股东增减持明细 |

**只剩 #8 客户与供应商集中度确实取不到** —— 实测 F10 经营分析只返回
主营构成/经营范围/经营评述，没有客户供应商字段。这个只在年报附注里，如实留白。

三处必须说清的坑：
- **#2 不存在一份统一的「前三大竞争对手」**。东财给的是三份榜单（成长性/估值/财务），
  实测歌尔股份这三份各 5 家、互相只有一两家重叠。所以对比用的是**行业平均与中值**，
  而不是挑两家公司点名 —— 硬点名会变成选择性比较。
- **股东增减持的数量单位是「万股」**。表里 `CHANGE_NUM=356.84` 对应的是 356.84 万股
  （与变动后持股 51124.857 万股、持股比例 14.66% 吻合），直接用会让规模差一万倍。
  方向取表格自带的 `DIRECTION` 字段，不用比例正负去猜。
- **「近一年无记录」是有效结论，不是取数失败**。歌尔股份近一年确实没有增减持公告，
  不能显示成「无数据」——「没有动作」和「拿不到数据」对判断的意义完全不同。

**几处要做对的地方：**

- **PE 五年分位是真五年**。用报告期末 + 法定披露滞后（避免前视偏差）还原每日
  TTM EPS，再逐日算 PE；剔除盈利接近零导致的失真值（歌尔 2023 年 PE 到过 880）
  并如实报告剔除天数。为拿够历史，财务取 24 期（约 6 年）、日线用本地库
  补到 1300 根 —— 库内是前复权序列（已验证：与实时前复权重叠区最大差 0.04%，
  且越久远与不复权的差异越大，符合复权累积特征）。
- **基期效应必须提示**。歌尔 2023 年净利 -37.8%，以它为基数算出的三年复合增速
  是 +90.3%，PEG 算出 0.24 显得「很便宜」。这是基数被压低的假象，
  所以结论后面强制附上说明，并用更稳定的营收口径做交叉验证。
- **支撑压力位只用被反复验证的价位**。zigzag 转折点聚类，按触碰次数 × 时间衰减
  排序；阈值随窗口长度调整（三年用 6%、三个月用 3.5%）。价格创新高时上方
  确实没有历史压力位 —— 这要用技术位补齐并**标明口径**，而不是假装没有。

栏目还提供「复制数据包」：13 个提示词原文 + 本系统算好的真实数据，
可直接粘给任意外部 AI，这样它也不需要编造这些数字。

### 箱体：按个股自身的节奏定交易日窗口

原来的箱体分析只在 30/60/120/250 四个固定窗口里挑一个。问题是**它用同一把
尺子量所有股票**，而每只股票的运作节奏差很远。窗口选错不是小误差，是结论反转。

以歌尔股份（002241.SZ）为例，同一时点、同一套算法，只换窗口：

| 窗口 | 箱底 | 箱顶 | 高度 | 当前位置 | 形态 | |
|---|---|---|---|---|---|---|
| 20 天 | 21.328 | 24.264 | 13.8% | 84% | 下降通道 | 太短，被判成通道 |
| 60 天 | 21.293 | 24.175 | 13.5% | 87% | 震荡箱体 | 固定窗口 |
| **70 天** | **21.301** | **24.175** | **13.5%** | **87%** | **震荡箱体** | **← 自适应** |
| 250 天 | 21.951 | 32.627 | **48.6%** | **17%** | 下降通道 | 结论完全反过来 |

250 天窗口说「在箱体 17%、接近底部」，70 天窗口说「在箱体 87%、接近顶部」。
差在哪？250 天里包含了一整段主跌，把它当成"箱底"毫无意义。

**做法分两步，顺序不能反：**

**第一步，量出这只股票自己的节奏。** 用 zigzag 在不同涨跌幅阈值下找波段转折点，
得到的是**嵌套**结构 —— 歌尔股份实测：

```ini
 3% 档   91 段   单边中位  4 天   完整往复  8 天    ← 噪声级别
 5% 档   47 段   单边中位  8 天   完整往复 16 天
10% 档   23 段   单边中位 17 天   完整往复 34 天    ← 用于定窗口
15% 档   11 段   单边中位 28 天   完整往复 56 天    ← 趋势级别
```

小档位抓到的是噪声，大档位抓到的是趋势，中间那档才是「在一段区间里来回磨」
的级别，也就是这只股票当下的操作节奏：**34 天一个完整往复**。

**第二步，让节奏定搜索范围，再在范围内按箱体质量选，取高分平台的中心。**
箱体窗口应覆盖 2~3 个完整往复：少于 2 个往复看到的只是一段单边行情，
多于 3 个又会把性质不同的几段行情混在一起。歌尔股份 → 65~105 天。
在这个范围内逐档打分，最高分出现在 75 天，但**不取最高分** ——
取高分平台 65~75 天的中心，即 **70 天**。

为什么不用最高分：47 档里最高分几乎总是一个孤立尖峰，那是拟合噪声拟合出来的。
真正有节奏的窗口，它左右邻居的分数也应该一样高，所以平台越宽越可信。

**箱体质量的打分维度**（每一项都对应箱体的一个必要条件）：

| 维度 | 含义 |
|---|---|
| `flat` | 上下沿是否躺平 —— 把窗口三等分各算一次边沿，看漂移了几个箱体高度。这是区分「水平箱体」和「斜着走的通道」最硬的指标 |
| `osc` | 价格是否反复穿越中轴（用**每 10 根的频率**，不用绝对次数） |
| `touch` | 上下沿是否被反复触碰，只有一边算不合格 |
| `inside` | 收盘价落在箱内的比例 |
| `notrend` | 回归斜率是否足够小 |
| `height_ok` | 箱体高度超过 20% 开始扣分，40% 归零 —— 37% 宽的"箱体"其实是趋势 |

**踩过的两个坑**（都是量了数据才发现的）：

1. **绝对触碰次数不能用**。240 根里碰到上沿 25 次，和 70 根里碰到 10 次，
   密度其实差不多，但前者根本不是箱体（实测 `flat=0`、几乎不穿越中轴）。
   长窗口下触碰次数几乎自动变多，所以改用频率。
2. **分数不能跨窗口长度直接比较**。长窗口经历更多行情演变，上下沿漂移更多，
   天然吃亏；短窗口相反。曾经在节奏区间推不出来时"退回全范围扫描"，
   结果平安银行选中了 20 天窗口（4.1% 高度、0.825 分）—— 那是短窗口占了
   尺度的便宜，不是这只股票的节奏。

**测不出就如实弃权。** 如果节奏超出箱体可刻画的 250 天上限（如平安银行
158 天一个往复），或者压根测不出节奏，系统会明说「当前没有可信的箱体」
并解释原因，而不是硬塞一个最不难看的窗口。平安银行、中国平安都会被这样拒绝。

面板上同时展示节奏阶梯、推导过程和「为什么是 N 日」，用户能看到依据而不是
只看到一个数字。想对比的话，面板上有「按个股节奏 / 固定窗口」切换。

### 一个顺带修掉的数据坑：新浪 K线是不复权的

做这个功能时发现，「请求更多历史反而拿到更少」：

```
limit= 800 → 实得  800 根（腾讯）   2023-06-07 起
limit=1000 → 实得  641 根（腾讯只回这么多，兜底没接上）
```

查下去是腾讯 `fqkline` 上游的行为：请求超过 800 根时**悄悄只回 641 根**
（请求 900/1000/1200 全都一样）。而链路是「第一个成功的源就返回」，
所以新浪明明能回 1000+ 根也没被用上。改成：拿到数据后如果还不够长，
就再多试一两家（最多 2 家）取更长的那个，同时把补数据时的单源预算收紧到 1 秒
（否则会白等东财 2.5 秒）。

更严重的是接上新浪之后暴露的问题：**新浪这个 K线接口只有不复权价格**。
实测与腾讯前复权的差异 ——
...
而箱体高度往往只有 10% 出头，这种系统性偏移足以挪动箱体边界和均线位置。
所以现在**请求复权数据时直接跳过新浪**：宁可少给数据，也不给价格口径不对的数据。

还有一处稳定性问题：节奏测量原本依赖「一共取了多少根 K线」。600519 在 641 根
下算出 56 天周期、800 根下算出 70 天 —— 因为更早的历史里波段更长，把中位数拉大了。
现在节奏固定在**最近 500 根**上测量，无论上层取多少历史，结果都可复现。
